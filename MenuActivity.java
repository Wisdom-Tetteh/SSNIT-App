...
public class MenuActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAuthListener = new FirebaseAuth.AuthStateListener() {
            @Override
            public void onAuthStateChanged(@NonNull FirebaseAuth firebaseAuth) { // Redirects unauthorized person to Main Activity
                if (firebaseAuth.getCurrentUser() == null){
                    Intent loginIntent = new Intent(MenuActivity.this,MainActivity.class);
                    loginIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK); // hit back button back to activity
    ...
    protected void onStart() {
        ...
        FirebaseRecyclerAdapter <Food,FoodViewHolder> FBRA = new FirebaseRecyclerAdapter<Food, FoodViewHolder>(
                com.tetteh1568.breadblog.Food.class,
            ...
            protected  void populateViewHolder(FoodViewHolder viewHolder, Food model, int position){
                ...
                final String food_key = getRef(position).getKey().toString(); //Selects the position or particulaar item the use clicked on
